...
getNodeModule node =
    case node of
        LoadOrgConfig -> "Database.load_organization_config"
        CheckContactCount -> "Database.get_total_contact_count"
        DetermineProfile -> "Size_profiles.auto_detect_profile"
        ApplyOverrides -> "Size_profiles.apply_config_overrides"
        CalculateCapacity -> "Size_profiles.load_balancing_for_profile"
        ProcessContact -> "Email_scheduler.calculate_anniversary_emails"
        CheckExclusions -> "Exclusion_window.check_exclusion_window"
        CalculatePriority -> "Types.priority_of_email_type"
        ScheduleEmail -> "Email_scheduler.schedule_email"
        BalanceLoad -> "Load_balancer.redistribute_overflow"
